...
import type { About as AboutType } from "@/typings";
...
export default function About({ about }: Props) {
  // we expect a single document; fall back to empty object if none
  const profile = about[0] || ({} as AboutType);
              ...
              <div className="bg-card rounded-3xl p-8 border border-white/5 relative overflow-hidden group">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative z-10">
                  ...
                  <img
                    ...
                    className="rounded-lg rotate-2 hover:rotate-0 transition-transform duration-300"
                    alt="Gear"
                  />
                  <img
                    src="https://images.unsplash.com/photo-1516035069371-29a1b244cc32?w=400"
                    className="rounded-lg -rotate-2 hover:rotate-0 transition-transform duration-300 translate-y-4"
                    alt="Field work"
                  />
                  <img
                    src="https://images.unsplash.com/photo-1452570053594-1b985d6ea890?w=400"
                    className="rounded-lg rotate-1 hover:rotate-0 transition-transform duration-300"
                    alt="Landscape"
                  />
                  <img
                    src="https://images.unsplash.com/photo-1480044965905-02098d419e96?w=400"
                    className="rounded-lg -rotate-1 hover:rotate-0 transition-transform duration-300 translate-y-6"
                    alt="Nature"
                  />
                </div>
              </div>
                ...
                {profile?.books?.map((book, idx) => (
                  <li key={idx} className="group">
                    <a
                      href={book.link}
                      target="_blank"
                      className="block p-4 rounded-xl bg-card border border-white/5 hover:border-primary/50 transition-colors">
                ...
                {profile?.articles?.map((article, idx) => (
                  <li key={idx}>
                    <a
                      ...
                      <span className="font-medium text-white underline decoration-primary/50 underline-offset-4">
                        {article.title}
                      </span>
              ...
              <div className="flex gap-4">
                {profile?.socialLinks?.map((link, idx) => (
                  <a
                    ...
                    href={link.url}
                    target="_blank"
                    className="w-12 h-12 rounded-full bg-card border border-white/10 flex items-center justify-center text-white hover:bg-primary hover:text-background hover:scale-110 transition-all duration-300">
                  ...
                  <>
                    <div className="w-12 h-12 rounded-full bg-card border border-white/10 flex items-center justify-center text-muted-foreground">
                      <Instagram />
                    </div>
                    <div className="w-12 h-12 rounded-full bg-card border border-white/10 flex items-center justify-center text-muted-foreground">
                      <Twitter />
